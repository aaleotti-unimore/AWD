...
def watchdog(project):
    ...
    observer.schedule(event_handler, path=path,
                      recursive=True)  # indica all'observer quale event handler usare, quale percorso osservare e di essere ricorsivo.
    observer.start()  # lancio
    time.sleep(1)  # attende

    chk = subprocess.check_output(
        "C:\Apache24\htdocs\AWD\AWD_Zanasi\matlab_script.bat \"%s\" \"%s\" " % (path, filepath),
        shell=True)  # lancia matlab
    logger.debug("%s" % chk)
...
class MyHandler(FileSystemEventHandler):
    ...
    def add_output(self, event):
        ...
        if event.src_path.endswith(".txt"):
            ...
            out, created = ProjectOutput.objects.get_or_create(project=self.project,
                                                               text_file=text_file_path)  # cerca o ottiene un oggett database di tipo ProjectOutput
